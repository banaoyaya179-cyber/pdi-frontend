...
const Navbar = () => {
  ...
  const liensCommuns = [
    ...
    { path: '/sites', label: '🏕️ Sites d\'accueil' },
    { path: '/carte', label: '🗺️ Carte' },
  ...
  return (
    <>
      ...
      <div className="text-white text-center py-2 px-4"
        style={{ display: "none" }}>
        "Derrière chaque dossier se cache une histoire. Derrière chaque intervention renaît un espoir.
        Merci de faire de votre engagement une source de dignité et d'avenir pour les personnes déplacées internes."
      </div>
            ...
            <ul className="navbar-nav me-auto">
              {liens.map(l => (
                <li className="nav-item" key={l.path}>
                  <Link
                    className={`nav-link ${location.pathname === l.path ? 'active fw-semibold' : ''}`}
                    to={l.path}
